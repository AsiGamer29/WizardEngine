    ...
    WizardModelData ModelImporter::Import(const std::string& filepath,
        ...
        for (unsigned int i = 0; i < scene->mNumMaterials; i++) {
            ...
            if (material->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
                ...
                std::string fullTexPath = baseDirectory + "/" + texPath.C_Str();

                // Try to load texture
                WizardTextureData texData = TextureImporter::Import(fullTexPath);
                if (!texData.data.empty()) {
                    std::string texFilename = "tex_" + std::to_string(i) + ".wzt";
